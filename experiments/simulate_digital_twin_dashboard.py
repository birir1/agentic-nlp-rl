# experiments/simulate_digital_twin_live_dashboard.py
import sys
import os
import numpy as np
...
from envs.digital_twin_env import DigitalTwinEnv  # Correct import
...
def animate_digital_twin(traj_positions, traj_valence, traj_weights, traj_comm, traj_coop,
                         goal=(0.9, 0.9), save_file="digital_twin_live_dashboard.mp4"):
    ...
    def update(frame):
        ...
        for aid, dot in agent_dots.items():
            ...
            val = np.clip(traj_valence[frame][aid], 0, 1)
            dot.set_data(pos[0], pos[1])
            dot.set_color((0.5 - 0.5*val, 0.5 + 0.5*val, 0.5))

            # Update label with ID, valence, weight
            label_text = f"{aid}\nval:{traj_valence[frame][aid]:.2f}\nw:{traj_weights[frame][aid]:.2f}"
    ...
    try:
        ...
        ani.save("digital_twin_live_dashboard.gif", writer="pillow", fps=2)
