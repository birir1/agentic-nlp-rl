# experiments/simulate_digital_twin_convergent_graphs.py
import sys
import os
import numpy as np
...
def animate_digital_twin(traj_positions, traj_valence, traj_weights, traj_comm, traj_coop,
                         goal=(0.9, 0.9), save_file="digital_twin_trails.mp4"):
    ...
    ax_env.set_xlim(0, 1)
    ax_env.set_ylim(0, 1)
    ax_env.set_title("Digital Twin Multi-Agent Coordination with Trails")
    ...
    def update(frame):
        ...
        for aid, dot in agent_dots.items():
            ...
            # Color = cooperation success (valence × weight)
            ...
            ax_env.plot(trail_array[:,0], trail_array[:,1], color=(0.2, 0.8, 0.2, 0.3))  # semi-transparent trail
    ...
    try:
        ...
        ani.save("digital_twin_trails.gif", writer="pillow", fps=2)

    plt.show()


def plot_convergence(traj_valence, traj_weights, save_file="convergence_plot.png"):
